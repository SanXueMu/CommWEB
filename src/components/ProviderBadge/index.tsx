...
export function providerColor(pid: string): string {
  const hash = Number.parseInt(contentHash(pid), 36)
  return PALETTE[hash % PALETTE.length]
}

export function ProviderBadge({ pid, name }: { pid: string; name?: string }) {
  if (!pid || pid === 'default') return null
  const color = providerColor(pid)
  return (
    <Tag
      style={{ marginInlineEnd: 0, fontSize: 10, lineHeight: '16px', padding: '0 4px', color, borderColor: color, background: 'transparent' }}
      title={`会员：${name ?? pid}`}
    >
      {(name ?? pid).slice(0, 6)}
    </Tag>
