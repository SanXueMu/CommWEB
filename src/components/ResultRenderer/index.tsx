...
/** 输出渲染：table（⚠️待审行高亮）/ text / json 三形态自动分派。 */
export function ResultRenderer({ output }: { output: unknown }) {
  ...
  if (kind === 'table') {
    ...
    return (
      <Table
        ...
        rowClassName={(row) => (rowNeedsReview(row as Record<string, unknown>) ? 'commweb-review-row' : '')}
