...
import type { ParsedSite } from '@/transfer/siteManifest'
import { useProviders } from '@/transfer/context'

export function useSiteCatalog(): { site: ParsedSite; isSiteLoading: boolean; refreshPrefs: () => void } {
  ...
  const site = useMemo(
    () => parseSite(manifest ?? DEFAULT_SITE, caps, pid),
