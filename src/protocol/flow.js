...
export function extractInputKeys(steps) {
    const keys = [];
 ...
 */
export function extractFlowFields(steps, toolSchemas) {
    const keys = extractInputKeys(steps);
    const firstToolByKey = {};
    for (const step of steps) {
        for (const template of Object.values(step.input)) {
            const text = String(template ?? '');
            for (const match of text.matchAll(/\{\{\s*input\.(\w+)\s*\}\}/g)) {
                if (!firstToolByKey[match[1]])
                    firstToolByKey[match[1]] = step.tool;
            }
        }
    }
    return keys.map((key) => {
        ...
        if (schema?.type === 'array')
            return { key, widget: 'tags' };
        if (FILE_FIELD_RE.test(key) || schema?.format === 'file')
            return { key, widget: 'file' };
        return { key, widget: 'text' };
    });
}
