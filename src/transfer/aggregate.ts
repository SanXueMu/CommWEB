...
import type { PipelineSummary, ToolSummary } from '@/api/types'
import { useProviders } from '@/transfer/context'

export function useAggregatedTools(): { tools: ToolSummary[]; loading: boolean; failedPids: string[] } {
...
export function useAggregatedPipelines(): { pipelines: PipelineSummary[]; loading: boolean; failedPids: string[] } {
  const { providers } = useProviders()
  const queries = useQueries({
    queries: providers.map((p) => ({
