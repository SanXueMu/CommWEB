    ...
    try {
        ...
        if (raw) {
            ...
            if (Array.isArray(list))
                return [DEFAULT_PROVIDER, ...list.filter((p) => p.id !== DEFAULT_PROVIDER.id)];
        }
    }
    catch {
        /* 损坏则回退 */
...
export const registry = {
    ...
    async probe(id) {
        ...
        const caps = { has_pipelines: false, has_files: false, has_runs: false, has_sse: true, has_statuses: false };
        let status = 'online';
        try {
